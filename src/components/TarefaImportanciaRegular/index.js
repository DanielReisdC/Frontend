...
const TarefaImportanciaRegular = () => {
  ...
  const fetchTarefasImportanciaRegular = async () => {
    try {
      ...
      setTarefasImportanciaRegular(response.data);
    } catch (error) {
      console.error("Erro ao buscar tarefas de importância Regular:", error);
    }
  };

  useEffect(() => {
    fetchTarefasImportanciaRegular();
  }, [token]);
  ...
  const tarefasRegularHoje = tarefasImportanciaRegular.filter((tarefa) => {
    return tarefa.importancia === "REGULAR" && tarefa.data_fim === dataHoje;
  });

  return (
    <>
      <ContainerPaiLembretes>
        {tarefasRegularHoje.map((tarefa) => (
...
export default TarefaImportanciaRegular;
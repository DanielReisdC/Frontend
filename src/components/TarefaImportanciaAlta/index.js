...
const TarefaImportanciaAlta = () => {
  ...
  const fetchTarefasImportanciaAlta = async () => {
    try {
      ...
      setTarefasImportanciaAlta(response.data);
    } catch (error) {
      console.error("Erro ao buscar tarefas de importância alta:", error);
    }
  };

  useEffect(() => {
    fetchTarefasImportanciaAlta();
  }, [token]);
  ...
  const tarefasUrgentesHoje = tarefasImportanciaAlta.filter((tarefa) => {
    return tarefa.importancia === "URGENTE" && tarefa.data_fim === dataHoje;
  });

  return (
    <>
      <ContainerPaiLembretes>
        {tarefasUrgentesHoje.map((tarefa) => (
...
export default TarefaImportanciaAlta;